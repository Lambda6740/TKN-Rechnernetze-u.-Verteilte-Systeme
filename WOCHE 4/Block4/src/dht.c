...
int servers_no, md5_first, ;
int successful_gets;

static void print_hash(unsigned int *hash) {
    printf("%d", hash);
    printf(".\n");
}
...
/* Compare two hashes */
static int compare_hash(unsigned int *hash1, unsigned int *hash2)
{
    int result, i;
    if (hash1 < hash2)
        return -1;
...
static int binary_search(struct server in_server, unsigned int hash)
{
    ...
    x = compare_hash(hash, in_server.pre_ID);
    y = compare_hash(hash, in_server.succ_ID);
...
static void get_md5_index(char *key, char *value)
{
    int i, x;
